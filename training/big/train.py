...
- Clases con >= 1000 ejemplos (solo las clases grandes).
- BATCH_SIZE = 16
- NUM_EPOCHS = 5
- Descongelar capas 8-11 de BERT (4 capas, mas fine-tuning).
- Learning rate diferencial: BERT bajo, clasificador alto.
"""
...
MIN_SAMPLES = 1000
MAX_SAMPLES = None  # Sin limite superior
MAX_LENGTH = 128
BATCH_SIZE = 16
NUM_EPOCHS = 5
LEARNING_RATE = 2e-5
DROPOUT_RATE = 0.1
UNFREEZE_LAYERS = [8, 9, 10, 11]  # 4 capas descongeladas


def main():
    print('=' * 60)
    print('ENTRENAMIENTO MODELO BIG')
    print(f'Clases: >= {MIN_SAMPLES} ejemplos')
    print('=' * 60)
    ...
    # Optimizer con learning rate diferencial
    bert_params = [p for n, p in model.bert.named_parameters() if p.requires_grad]
    classifier_params = list(model.classifier.parameters())

    optimizer = torch.optim.AdamW([
        {'params': classifier_params, 'lr': 1e-4},    # Clasificador: LR alto
        {'params': bert_params, 'lr': LEARNING_RATE},  # BERT: LR bajo
    ], weight_decay=0.01)
    ...
    print('\n' + '=' * 60)
    print('ENTRENAMIENTO BIG COMPLETADO')
